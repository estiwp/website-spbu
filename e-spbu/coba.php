...
if (isset($_GET['Lokasi'])) {
  $colname_cari = (get_magic_quotes_gpc()) ? $_GET['Lokasi'] : addslashes($_GET['Lokasi']);
}
...
$maxRows_cari2 = 10;
$pageNum_cari2 = 0;
if (isset($_GET['pageNum_cari2'])) {
  $pageNum_cari2 = $_GET['pageNum_cari2'];
}
$startRow_cari2 = $pageNum_cari2 * $maxRows_cari2;

$colname_cari2 = "-1";
if (isset($_POST['txt_search2'])) {
  $colname_cari2 = (get_magic_quotes_gpc()) ? $_POST['txt_search2'] : addslashes($_POST['txt_search2']);
}
mysql_select_db($database_koneksi, $koneksi);
$query_cari2 = sprintf("SELECT * FROM pencarian WHERE Fasilitas = '%s'", $colname_cari2);
$query_limit_cari2 = sprintf("%s LIMIT %d, %d", $query_cari2, $startRow_cari2, $maxRows_cari2);
$cari2 = mysql_query($query_limit_cari2, $koneksi) or die(mysql_error());
$row_cari2 = mysql_fetch_assoc($cari2);

if (isset($_GET['totalRows_cari2'])) {
  $totalRows_cari2 = $_GET['totalRows_cari2'];
} else {
  $all_cari2 = mysql_query($query_cari2);
  $totalRows_cari2 = mysql_num_rows($all_cari2);
}
$totalPages_cari2 = ceil($totalRows_cari2/$maxRows_cari2)-1;
...
if (isset($_POST['txt_search2'])) 
{
  	$searchword = $_POST['txt_search2'] ;
	$query_cari2 = "SELECT * FROM pencarian WHERE Fasilitas LIKE '%".$searchword."%'";
}
else
{
	$query_cari2 = "SELECT * FROM pencarian"; 
}
...
if (isset($_POST['txt_search'])) 
{
	...
	$query_cari = "SELECT * FROM pencarian WHERE Lokasi LIKE '%".$searchword."%'";
}
else
{
...
<body>
<div id="container">
<div id="wb_Shape1" style="position:absolute;left:0px;top:382px;width:800px;height:262px;z-index:4;">
...
<div id="wb_Carousel1" style="position:absolute;left:1px;top:382px;width:799px;height:263px;z-index:10;overflow:hidden">
<div id="Carousel1" style="position:absolute; top: 1px; background-image: url(images/SPBU.jpg); layer-background-image: url(images/SPBU.jpg); border: 1px none #000000;">
<div class="frame">
<div id="wb_Image1" style="position:absolute;left:0;top:-1px;width:250px;height:188px;z-index:0;">
...
<div id="wb_CssMenu2" style="position:absolute;left:0px;top:332px;width:219px;height:50px;z-index:11;">
<ul>
...
<div style="position:absolute; text-align:center; left:277px; top:672px; width:799px; height:378px; z-index:13; background-color: #FFFFFF; layer-background-color: #FFFFFF; border: 1px none #000000;" title="">
...
<div class="indent1">
  ...
  <form method="post" name="form1">
    <div align="left">
      ...
      <p>&nbsp;</p>
      <p>&nbsp;</p>
      <div align="center">Lokasi
        <input name="txt_search" type="text" id="txt_search">
        <input name="btn_search" type="submit" id="btn_search" value="Cari">
Fasilitas
<input name="txt_search2" type="text" id="txt_search2">
<input name="btn_search2" type="submit" id="btn_search2" value="Cari2">
</div>
      <p align="center"> </p>
        ...
        <?php if ($totalRows_cari > 0) { // Show if recordset not empty ?>
          ...
          <br>
          <br>
          <?php if ($totalRows_cari2 > 0) { // Show if recordset not empty ?>
            <table border="1" align="center">
              <tr>
                <td>No</td>
                <td>Lokasi</td>
              ...
              <?php do { ?>
                <tr>
                  <td><?php echo $row_cari2['No']; ?></td>
                  <td><?php echo $row_cari2['Lokasi']; ?></td>
                  <td><?php echo $row_cari2['Fasilitas']; ?></td>
                  <td><?php echo $row_cari2['Jam Operasional']; ?></td>
                  <td><?php echo $row_cari2['Jenis Produk']; ?></td>
                </tr>
                <?php } while ($row_cari2 = mysql_fetch_assoc($cari2)); ?>
            </table>
            <?php } // Show if recordset not empty ?><?php } // Show if recordset not empty ?>
<p align="center">&nbsp;</p>
        <table border="0" width="50%" align="center">
          <tr>
            <td align="center"><a href="coba.php">Show All</a> 
...
Total Record: <?php echo $totalRows_cari ?> </td>
    ...
    <div align="center"></div><div align="center"></div><blockquote>&nbsp;</blockquote>
  </form>
</div>
</div>
